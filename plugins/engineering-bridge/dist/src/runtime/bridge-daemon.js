...
async function main() {
    ...
    if (process.argv[3] !== undefined && !ownerReadOnly)
        throw new Error("Invalid Bridge runtime owner mode.");
    const owner = await startBridgeOwner(configPath, async (readOnly) => {
        const runtime = await createBridgeRuntime(configPath, readOnly);
        return {
            createServer: async (sessionReadOnly) => {
                if (!sessionReadOnly)
            ...
            isBusy: () => runtime.service.hasActiveWork() || runtime.controlledPatches.hasActiveWork() ||
                [...runtime.executionWorkspaceIds].some((workspaceId) => {
                    try {
                        return runtime.collaboration.list(workspaceId).some(({ state }) => state === "queued" || state === "running" || state === "interrupting");
                    }
                    catch {
                        return false;
                    }
