...
export async function readCodexProjectReferences(configPath) {
    ...
    if (basename(canonical) !== "config.toml")
        throw new CoreError("WORKSPACE_PRECONDITION_FAILED");
    return parseCodexProjectReferences(await readFile(canonical, "utf8"));
}
export function parseCodexProjectReferences(source) {
    const references = [];
    let current;
    const flush = () => {
    ...
    for (const line of source.split(/\r?\n/)) {
        ...
        if (section !== null) {
            ...
            try {
                ...
                current = { path: literal.startsWith('"') ? JSON.parse(literal) : literal.slice(1, -1) };
            }
            catch {
                current = undefined;
            }
