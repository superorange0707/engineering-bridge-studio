...
import { acquireControlPlaneRuntimeLease, applyControlPlaneTransaction } from "./control-plane-transaction.js";
...
async function readArtifact(directory, name, expectedSha256) {
    ...
    try {
        ...
        if (uid === undefined || !metadata.isFile() || metadata.uid !== uid || metadata.nlink !== 1 ||
            (metadata.mode & 0o022) !== 0 || metadata.size > MAX_ARTIFACT_BYTES)
            fail();
        const contents = await handle.readFile().catch(() => fail());
        if (contents.length !== metadata.size || sha256(contents) !== expectedSha256)
            fail();
        return contents;
    }
    finally {
...
async function trustedConfigPath() {
    const modulePath = await realpath(fileURLToPath(import.meta.url)).catch(() => fail());
    const stackRoot = resolve(dirname(modulePath), "../../../..");
    if (modulePath !== join(stackRoot, "engineering-bridge-router", "dist", "src", "workspaces", "apply-identity-v2-migration.js"))
        fail();
    return join(stackRoot, "config", "workspaces.json");
}
export async function applyFrozenIdentityV2Migration(request, hooks) {
    if (request.proposalId !== PROPOSAL_ID || request.confirmation !== "APPLY" ||
        !isAbsolute(request.artifactDirectory) || normalize(request.artifactDirectory) !== request.artifactDirectory)
        fail();
    const artifactDirectory = await realpath(request.artifactDirectory).catch(() => fail());
    if (artifactDirectory !== request.artifactDirectory)
        fail();
...
async function main() {
    ...
    if (proposalId === undefined || confirmation === undefined || artifactDirectory === undefined || extra !== undefined)
        fail();
