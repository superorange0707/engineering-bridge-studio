...
import { DEFAULT_CODEX_MODEL_REGISTRY, registryAvailability, requireModelRegistry, taskMetadata } from "./codex-model-registry.js";
...
function object(value) { return typeof value === "object" && value !== null; }
function codexVersion(initializeResult) {
...
function startCodex(workspaceRoot, startProcess, hostEnvironment, platform) {
    const options = {
    ...
    if (resolved.kind === "direct")
        return startProcess(resolved.executable, ["app-server", "--stdio"], options);
...
function secretRiskPath(path) {
    return path.toLowerCase().replaceAll("\\", "/").split("/").some((part) => /^\.env(?:\.|$)/u.test(part) || SECRET_PATH_PART.test(part));
}
function withheld(reason, exitCode) {
    return { state: "withheld", ...(exitCode === undefined ? {} : { exit_code: exitCode }), reason };
}
function commandResult(item, status, workspaceRoot) {
    const exitCode = typeof item.exitCode === "number" && Number.isSafeInteger(item.exitCode)
        ? item.exitCode
        : undefined;
    if (status !== "completed" || exitCode !== 0)
        return withheld("non_success", exitCode);
    ...
    try {
        ...
        if (typeof item.cwd !== "string" || !isAbsolute(item.cwd))
    ...
    for (const action of item.commandActions) {
        if (!object(action) || !["read", "listFiles", "search"].includes(action.type)) {
            return withheld("unsafe_action", exitCode);
        }
    ...
    if (typeof item.aggregatedOutput !== "string")
        return withheld("missing_output", exitCode);
    if (UNSAFE_OUTPUT.test(item.aggregatedOutput))
        return withheld("unsafe_output", exitCode);
...
export async function probeCodexCapabilities(workspaceRoot, startProcess = spawn, hostEnvironment = process.env, platform = process.platform, registry = DEFAULT_CODEX_MODEL_REGISTRY) {
    let child;
    try {
    ...
    child.stdout.on("data", (chunk) => {
        ...
        while ((newline = buffer.indexOf("\n")) >= 0) {
            if (newline > MAX_JSONL_FRAME) {
                fail(new CoreError("CODEX_PROTOCOL_ERROR"));
                return;
            }
            ...
            if (!line)
                continue;
            if (Buffer.byteLength(line, "utf8") > MAX_JSONL_FRAME) {
                fail(new CoreError("CODEX_PROTOCOL_ERROR"));
                return;
            }
            let message;
            ...
            if (!object(message) || typeof message.id !== "number" || !("result" in message || "error" in message))
                continue;
        ...
        if (Buffer.byteLength(buffer, "utf8") > MAX_JSONL_FRAME)
            fail(new CoreError("CODEX_PROTOCOL_ERROR"));
    });
    ...
    try {
        ...
        const roles = registryAvailability(await call("model/list", { includeHidden: true, limit: 100 }), registry);
        return {
...
export class CodexExecutor {
    ...
    nextId = 1;
    pending = new Map();
    constructor(workspaceRoot, startProcess = spawn, hostEnvironment = process.env, platform = process.platform, registry = DEFAULT_CODEX_MODEL_REGISTRY) {
    ...
    async execute(request) {
        ...
        const unavailable = () => finish(failure("CODEX_UNAVAILABLE"));
        ...
        child.stdout.on("data", (chunk) => {
            ...
            while ((newline = buffer.indexOf("\n")) >= 0) {
                ...
                if (typeof message.id === "number" && ("result" in message || "error" in message)) {
                    ...
                    if (waiter) {
                        ...
                        if (waiter.method === "turn/start" && object(message.result) && object(message.result.turn) &&
                            ...
                            this.turnId = message.result.turn.id;
                ...
                if (typeof message.method !== "string" || !object(message.params)) {
                    finish(failure("CODEX_PROTOCOL_ERROR"));
                    return;
                }
                if (typeof message.params.threadId === "string" && message.params.threadId !== this.threadId)
                    continue;
                if (typeof message.params.turnId === "string" && message.params.turnId !== this.turnId)
                    continue;
                if (message.method === "turn/started") {
                    const turn = object(message.params.turn) ? message.params.turn : message.params;
                    if (message.params.threadId === this.threadId && typeof turn.id === "string" && (!this.turnId || turn.id === this.turnId))
                ...
                if ((message.method === "item/started" || message.method === "item/completed") && item) {
                    ...
                    if (id && (item.type === "commandExecution" || item.type === "fileChange")) {
                        ...
                        while (evidence.size + (evidenceDropped > 0 ? 1 : 0) > MAX_EVIDENCE) {
                            evidence.delete(evidence.keys().next().value);
                            evidenceDropped += 1;
                        }
                        try {
                ...
                if (message.method === "turn/completed") {
                    const turn = object(message.params.turn) ? message.params.turn : message.params;
                    if (message.params.threadId !== this.threadId || !this.turnId || turn.id !== this.turnId)
                        continue;
                    ...
                    if (status === "failed")
                        ...
                        finish({ kind: "interrupted", output, ...common });
                    else if (status === "completed")
                        finish({ kind: "completed", output, ...common });
                    else
                        finish(failure("CODEX_PROTOCOL_ERROR"));
                    if (settled)
                        return;
        ...
        try {
            ...
            if (request.threadId)
                ...
                threadParams.serviceName = "engineering-bridge-auto-router";
            ...
            if (!object(threadResult) || !object(threadResult.thread) || typeof threadResult.thread.id !== "string")
                throw new Error();
            if (threadResult.model !== role.model)
            ...
            if (this.startedTurnId !== this.turnId)
                this.startedTurnId = undefined;
        }
        catch (error) {
            if (!settled) {
