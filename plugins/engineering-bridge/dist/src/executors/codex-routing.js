...
function startsWithPositiveAction(text) {
...
function appendRoutingSpans(spans, text) {
    for (const segment of text.split(RESET_BOUNDARY)) {
        let guarded = false;
        let parts = [];
        const flush = () => {
            if (parts.some((part) => part.trim()))
                spans.push({ text: parts.join(", "), guarded });
            parts = [];
        };
...
function conditionalOutcomeIsGuard(text) {
    if (CONDITIONAL_REJECTION.test(text))
        return true;
    const stop = /\bstop\b|停止/u.exec(text);
    const consequenceStart = /[,，]/u.exec(text)?.index;
    if (stop === null || consequenceStart === undefined)
        return true;
    return !HIGH_RISK_ACTION.test(text.slice(consequenceStart + 1, stop.index));
}
function routingSpans(instruction) {
    const spans = [];
...
function highRiskFactors(spans) {
    const matched = [];
    const ignored = [];
...
function positiveFactors(spans, expression, prefix) {
    return unique(spans.flatMap(({ text, guarded }) => guarded
        ? []
        : matches(expression, text).map(({ value }) => `${prefix}:${value}`)));
}
export function routeCodexTask(routing = "auto", instruction) {
    if (routing !== "auto") {
        return {
...
export function routingTransition(parent, child) {
    const rank = {
        implementer: 0,
        local_lead: 1,
        repo_principal: 2
