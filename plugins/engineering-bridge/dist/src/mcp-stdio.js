...
import { acquireControlPlaneRuntimeLease, recoverControlPlaneTransactions } from "./workspaces/control-plane-transaction.js";
...
const RunTaskValidationSchema = RunTaskPublicSchema.superRefine(({ executor, routing, parent_task_id, handoff_snapshot }, context) => {
...
const WorkspaceDiagnosticsValidationSchema = WorkspaceDiagnosticsPublicSchema.superRefine(({ workspace_id, project_path }, context) => {
...
function deserializeStableObjectIdentity(identity) {
    return {
...
function sameStableObjectIdentity(left, right) {
...
async function registerConfiguredWorkspaceSeeds(parsed, catalog) {
    for (const seed of parsed.workspaces) {
        if (parsed.excluded_workspace_ids.includes(seed.workspace_id))
            continue;
        await catalog.registerOnce(seed.current_path, {
            ...
            filesystem: {
                ...
                ...(seed.filesystem_identity.stable_object_identity === undefined
                    ? {}
                    : { stableObjectIdentity: deserializeStableObjectIdentity(seed.filesystem_identity.stable_object_identity) })
            },
            ...
            ...(seed.git_identity === undefined ? {} : {
                ...
                repository: {
                    ...
                    ...(seed.git_identity.repository_identity.stable_object_identity === undefined
                        ? {}
                        : { stableObjectIdentity: deserializeStableObjectIdentity(seed.git_identity.repository_identity.stable_object_identity) }),
...
function registerCatalogEntries(catalog, registry) {
    for (const entry of catalog.identityEntries()) {
        if (entry.source === "approved")
            registry.registerApproved(entry.id, entry.root, entry.allowWrite);
        else
            registry.registerManaged(entry.id, entry.root, entry.allowWrite);
    }
}
export async function createBridgeRuntime(configPath, localReadOnly) {
    ...
    try {
        ...
        if (localReadOnly) {
            for (const seed of parsed.workspaces) {
                ...
                const sameFilesystem = record?.filesystem.fingerprint === seed.filesystem_identity.fingerprint &&
                    ...
                    sameStableObjectIdentity(record.filesystem.stableObjectIdentity, seed.filesystem_identity.stable_object_identity);
                const sameGit = seed.git_identity === undefined
                    ...
                    : record?.gitTopLevel === seed.git_identity.git_top_level &&
                        ...
                        sameStableObjectIdentity(record.repository?.stableObjectIdentity, seed.git_identity.repository_identity.stable_object_identity);
        ...
        const makeOnboarding = (readOnly) => new WorkspaceOnboardingService(registry, catalog, parsed.managed_roots.map(({ root, permission_policy }) => ({
            root,
            allowWrite: permission_policy.allow_write
        })), undefined, undefined, undefined, undefined, undefined, () => readCodexProjectReferences(parsed.codex_projects.source_file), parsed.excluded_workspace_ids, parsed.codex_projects.permission_policy.allow_write, readOnly, parsed.codex_projects.auto_onboard);
        ...
        const controlledPatches = new ControlledPatchService(registry, service, undefined, `${configPath}.controlled-patches.json`, DEFAULT_CODEX_MODEL_REGISTRY, async (workspaceId) => {
        ...
        const collaboration = new CollaborationRunService(join(stateRoot, "collaboration"), registry, undefined, async (workspaceId) => { await onboarding.ensureAvailable(workspaceId); });
        ...
        const ensureWritable = async () => {
            if (writableEnabled)
                return;
            if (writableUpgrade !== undefined)
                return writableUpgrade;
            const upgrade = (async () => {
            ...
            try {
                ...
                if (writableUpgrade === upgrade)
                    writableUpgrade = undefined;
            }
        };
        runtime = {
            configPath,
...
export function createBridgeServer(runtime, localReadOnly = runtime.localReadOnly) {
    const { configPath, parsed, catalog, onboarding: runtimeOnboarding, readOnlyOnboarding, service, controlledPatches, projectInstructions, collaboration, executionWorkspaceIds } = runtime;
    ...
    server.registerTool("collaboration_history", {
        ...
        try {
            return jsonContent(historyPage(collaboration.list(workspace_id), offset, limit));
        }
        catch (error) {
            return { isError: true, ...jsonContent({ error: serializeError(error) }) };
        }
    ...
    server.registerTool("collaboration_artifact", {
        ...
        try {
            return jsonContent(artifactPage(await collaboration.readArtifact(run_id, path), offset_bytes, max_bytes));
        }
        catch (error) {
            return { isError: true, ...jsonContent({ error: serializeError(error) }) };
        }
    ...
    server.registerTool("run_task", {
        ...
        const { workspace_id, instruction, executor, routing, parent_task_id, handoff_snapshot } = RunTaskValidationSchema.parse(input);
        if (catalog.get(workspace_id) !== undefined)
            await onboarding.ensureAvailable(workspace_id);
    ...
    server.registerTool("control_task", {
        ...
        if (service.taskView(task_id) === undefined)
            return unknownTask();
        try {
    ...
    server.registerTool("generate_controlled_patch", {
        ...
        const { workspace_id, change_request, routing, parent_task_id, handoff_snapshot } = GeneratePatchValidationSchema.parse(input);
        try {
            if (catalog.get(workspace_id) !== undefined)
                await onboarding.ensureAvailable(workspace_id);
    ...
    server.registerTool("bridge_capabilities", {
        ...
        try {
            ...
            return { isError: true, ...jsonContent({
                    ...
                    roles: Object.fromEntries(Object.entries(DEFAULT_CODEX_MODEL_REGISTRY).map(([role, config]) => [role, { ...config, available: false, maxSupported: false }])),
                    default_routing: "auto",
                    max_ready: false,
                    workspace_identity: "stable-id+filesystem-evidence+optional-git",
                    git_optional: true,
...
async function main() {
    ...
    if (configPath === undefined)
        throw new Error("Workspace configuration path is required.");
