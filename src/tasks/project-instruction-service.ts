...
import { ControlledPatchService } from "./controlled-patch-service.js";
import type { ProjectInstructionTarget } from "./controlled-patch-service.js";
...
export type ProjectInstructionStatus = "READY" | "HOLD_NEEDS_PROJECT_DECISION" | "ALREADY_CURRENT";

export interface ProjectInstructionPreparation {
  readonly status: ProjectInstructionStatus;
  readonly workspace_id: string;
  readonly evidence_sha256: string;
  readonly audited_files: readonly string[];
  readonly evidenced_commands: readonly string[];
  readonly proposed_targets: readonly string[];
  readonly task_id?: string | undefined;
  readonly base_head?: string | null | undefined;
  readonly hold_reason?: "AMBIGUOUS_MANAGED_BLOCK" | "EXISTING_INSTRUCTION_CONFLICT" |
    "UNSAFE_INSTRUCTION_TARGET" | undefined;
}
...
type ManagedBlock = { readonly status: "absent" | "invalid" } | ValidManagedBlock;
...
export class ProjectInstructionService {
  ...
  async prepare(workspaceId: string): Promise<ProjectInstructionPreparation> {
    ...
    let existing: readonly ExistingInstruction[];
    try {
    ...
    if (conflict) return hold(workspaceId, evidence, "EXISTING_INSTRUCTION_CONFLICT");

    const targets = existing.flatMap((instruction, index): ProjectInstructionTarget[] => {
      const block = instruction.path === "AGENTS.md"
        ? agentsManagedBlock(evidence)
        : plansManagedBlock(evidence);
      const state = managed[index]!;
      if (state.status === "valid" && state.content === block) return [];
    ...
    if (targets.length === 0) return summary("ALREADY_CURRENT", workspaceId, evidence, []);
...
function hold(
  ...
  reason: ProjectInstructionPreparation["hold_reason"]
): ProjectInstructionPreparation {
...
async function collectEvidence(root: string): Promise<BoundedEvidence> {
  ...
  for (const name of MANIFEST_NAMES) {
    ...
    if (name === "package.json") {
      try {
        ...
        if (isObject(value)) packageJson = value;
      } catch {
...
function packageManager(packageJson: Record<string, unknown>, entries: readonly string[]): "npm" | "pnpm" | "yarn" {
...
function technologySummary(manifests: readonly string[], packageJson: Record<string, unknown> | undefined): string {
  const facts: string[] = [];
  if (manifests.includes("package.json")) facts.push("Node.js package metadata (package.json)");
  if (manifests.includes("pyproject.toml")) facts.push("Python project metadata (pyproject.toml)");
  if (manifests.includes("Cargo.toml")) facts.push("Rust package metadata (Cargo.toml)");
  if (manifests.includes("go.mod")) facts.push("Go module metadata (go.mod)");
  if (manifests.some((name) => name === "pom.xml" || name.startsWith("build.gradle"))) facts.push("JVM build metadata");
  if (manifests.includes("Package.swift")) facts.push("Swift package metadata (Package.swift)");
  if (packageJson !== undefined) {
    ...
    for (const framework of ["next", "react", "vue", "svelte", "vite", "typescript"]) {
      if (dependencies.includes(framework)) facts.push(`${framework} dependency`);
    }
  }
...
function agentsManagedBlock(evidence: BoundedEvidence): string {
...
function plansManagedBlock(evidence: BoundedEvidence): string {
  return `${MANAGED_BLOCK_BEGIN}\n<!-- evidence-sha256:${evidence.digest} -->\n\n## Objective\n\n## User-visible outcome\n\n## Evidence and current behavior\n\n- Project evidence digest: \`${evidence.digest}\`\n\n## Approved design\n\n## Scope\n\n### Allowed\n\n### Forbidden\n\n## Invariants\n\n## Ordered milestones\n\n1. \n2. \n3. \n\n## Verification\n\n## Checkpoints\n\n## Escalation conditions\n\n## Decisions made\n\n| Date | Decision | Reason | Evidence |\n|---|---|---|---|\n\n## Status\n\n## Remaining risks\n${MANAGED_BLOCK_END}`;
}

function newInstructionContent(path: ExistingInstruction["path"], block: string): string {
...
function replaceManagedBlock(existing: string, current: ValidManagedBlock, replacement: string): string {
  return `${existing.slice(0, current.start)}${replacement}${existing.slice(current.end)}`;
}

function parseManagedBlock(content: string): ManagedBlock {
  if (!content.includes(MANAGED_BLOCK_TOKEN)) return { status: "absent" };
  const tokenCount = content.split(MANAGED_BLOCK_TOKEN).length - 1;
  const markers = [...content.matchAll(MANAGED_BLOCK_MARKER)];
  if (tokenCount !== 2 || markers.length !== 2) return { status: "invalid" };
...
function contentOutsideManagedBlock(content: string, block: ManagedBlock): string {
  return block.status === "valid"
    ? `${content.slice(0, block.start)}${content.slice(block.end)}`
    : content;
}

function hasInstructionConflict(content: string): boolean {
  return PORTABILITY_CONFLICT.test(content) || SECRET_RISK.test(content);
}

async function readInstruction(root: string, path: "AGENTS.md" | "PLANS.md"): Promise<ExistingInstruction> {
  return { path, content: await readBoundedOrdinaryFile(root, path) };
}

async function readBoundedOrdinaryFile(root: string, relativePath: string): Promise<string | undefined> {
  ...
  try {
    ...
    if (canonical !== target) throw new CoreError("WORKSPACE_BOUNDARY_VIOLATION");
...
async function requireCanonicalDirectory(root: string): Promise<void> {
...
async function isGitTracked(root: string, path: string): Promise<boolean> {
  try {
